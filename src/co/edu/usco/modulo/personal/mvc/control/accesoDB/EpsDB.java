 ...
 *
 */
public class EpsDB implements conexion {
	private Logger imp ;
	/* (non-Javadoc)
...
//			 usuarioOb admin= (usuarioOb) param[1];
			...
			sql.append("SELECT [eps_codigo],[eps_nombre]   ");
			sql.append("FROM [eps]  with(nolock)  ");
//			sql.append("where cert_estado=1  ");
			...
			while(resul.next()){
				...
				elemento.setEps_codigo(resul.getInt(1));
				elemento.setEps_nombre(resul.getString(2));
